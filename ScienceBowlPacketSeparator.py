...
#Move separated packets into a folder
...
#9A_MS_Reg_2016 Q3 Bonus: Question spanning multiple lines, it never finds the problem statement
...
with open("9A_MS_Reg_2016.txt", 'r') as questionPacket:
...
def createQuestionArray(contents):
    ...
    for i in range(0,len(contents)):
        ...
        if newQuestion.questionTypeA == "TOSS-UP" or newQuestion.questionTypeA == "BONUS":
            ...
            newQuestion.questionCategory = readUntilString(contents[i+1], contents[i+1].find(")") + 1, "–") #Start after the parentheses
...
#Stores all categories of questions
categories = []

#Create separate files for each category
for q in questions:
    if (q.questionCategory.replace(" ","") in categories):
        with open(q.questionCategory.replace(" ","") + ".txt", 'a') as file: #We remove all spaces to avoid conflict with weird spacing Ex. " Math " vs "Math "
            file.write(q.getStringForm())
    else:
        try: #Throws exception if file already exists
            categories.append(q.questionCategory.replace(" ","")) 
            with open(q.questionCategory.replace(" ","") + ".txt", 'x') as newFile:
                newFile.write(q.getStringForm())
        except:
            with open(q.questionCategory.replace(" ","") + ".txt", 'w') as file: #We remove all spaces to avoid conflict with weird spacing Ex. " Math " vs "Math "
                file.write(q.getStringForm())
